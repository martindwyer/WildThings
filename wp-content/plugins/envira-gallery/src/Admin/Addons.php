...
class Addons {
	...
	public function addons_page() {
		...
		<div id="addon-heading" class="subheading clearfix">
			...
			<form id="add-on-search">
				...
				<input id="add-on-searchbox" name="envira-addon-search" value="" placeholder="<?php esc_attr_e( 'Search ' . ( apply_filters( 'envira_whitelabel', false ) ? '' : 'Envira ' ) . 'Addons', 'envira-gallery' ); ?>" />
	...
	public function addons_content() {
		...
		if ( ! $addons ) {
			...
			<form id="envira-addons-refresh-addons-form" method="post">
				...
				<p>
					<a href="<?php echo esc_url( wp_unslash( $_SERVER['REQUEST_URI'] ) ); ?>" class="button button-primary"><?php esc_html_e( 'Refresh Addons', 'envira-gallery' ); ?></a>
				</p>
			</form>
			<?php
			return;
	...
	public function get_addons() {
		...
		foreach ( (array) $addons as $i => $addon ) {
			...
			if (
				empty( $type ) ||
				( in_array( 'advanced', $addon->categories ) && 'gold' !== $type && 'platinum' !== $type && 'pro' !== $type && 'ultimate' !== $type && 'agency' !== $type ) ||
				( in_array( 'basic', $addon->categories ) && ( 'silver' !== $type && 'gold' !== $type && 'platinum' !== $type && 'plus' !== $type && 'pro' !== $type && 'ultimate' !== $type && 'agency' !== $type ) )
			) {
				// Unlicensed.
	...
	public function refresh_addons_action() {
		...
		return isset( $_POST['envira-gallery-refresh-addons-submit'] ) && wp_verify_nonce( wp_unslash( $_POST['envira-gallery-refresh-addons'] ), 'envira-gallery-refresh-addons' );
