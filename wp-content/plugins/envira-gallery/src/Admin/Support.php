...
class Support {
	...
	public function __construct() {
		...
		if ( isset( $_GET['action'] ) && $_GET['action'] == 'edit-gallery-settings' ) {
			add_action( 'envira_view_gallery_settings', array( $this, 'envira_display_gallery_settings' ) );
		}
	...
	public function envira_display_gallery_settings() {
		...
		if ( empty( $_GET['page'] ) || $_GET['page'] != 'envira-gallery-support-tools' || ( $_GET['page'] == 'envira-gallery-support-tools' && empty( $_REQUEST ) ) ) {
			return;
		}

		if ( $_REQUEST['action'] != 'view-gallery-settings' ) {
			return;
		}

		if ( ! isset( $_REQUEST['gallery_id'] ) || intval( $_REQUEST['gallery_id'] ) == 0 ) {
			return;
		}
	...
	public function envira_edit_gallery_settings() {
		...
		if ( ! isset( $_POST['action'] ) || $_POST['action'] != 'edit-gallery-settings' ) {
			return;
		}
		...
		// $oringial_settings = json_decode( envira_get_gallery_config( $gallery_id ), true );
							...
							<ul>
								...
								if ( ! empty( $galleries ) ) {
									...
									foreach ( $galleries as $gallery ) {
										...
										$color = isset( $gallery['severity'] ) && $gallery['severity'] !== 'note' ? 'red' : 'orange';
										$label = isset( $gallery['severity'] ) && $gallery['severity'] !== 'note' ? 'Warning' : 'Note';
								...
								if ( $this->total_albums() > 0 ) {
									...
									if ( ! empty( $albums ) ) {
										...
										foreach ( $albums as $album ) {
											...
											$color = isset( $album['severity'] ) && $album['severity'] !== 'note' ? 'red' : 'orange';
											$label = isset( $album['severity'] ) && $album['severity'] !== 'note' ? 'Warning' : 'Note';

											?>
											<?php if ( ! empty( $album['gallery_count'] ) ) { ?>
									<li><span class="alert" style="font-weight: 600; color:<?php echo $color; ?>;"><?php echo $label; ?>:</span> <a href="<?php echo $album['link']; ?>"><?php echo $album['title']; ?></a> has <?php echo $album['gallery_count']; ?> galleries.</li>
									<?php } else { ?>
										<li><span class="alert" style="font-weight: 600; color:<?php echo $color; ?>;"><?php echo $label; ?>:</span> <a href="<?php echo $album['link']; ?>"><?php echo $album['title']; ?></a> <?php echo $album['issue_text']; ?></li>
									<?php } ?>
											<?php
										}
									}
