...
class Admin_Container {
	...
	public function __construct() {
		...
		add_action( 'admin_init', array( $this, 'maybe_activate_gutenberg' ), 99 );

	}

	/**
	 * Checks to see if (1) Gutenberg exists in the install and (2) if global var set to disable Envira loading of Gutenberg
	 *
	 * @since 1.8.5
	 *
	 * @return void
	 */
	public function maybe_activate_gutenberg() {

		if (
			( ! defined( 'ENVIRA_GUTENBERG' ) || ( defined( 'ENVIRA_GUTENBERG' ) && ENVIRA_GUTENBERG !== false ) ) &&
	...
	public function fix_conflicts() {
		...
		if ( 'post.php' !== $pagenow && isset( $_GET['post'] ) && 'envira' !== get_post_type( $_GET['post'] ) ) {
			return;
		}

		if ( class_exists( 'WRGF' ) ) {
			global $WRGF;
			remove_action( 'admin_print_scripts-post.php', array( $WRGF, 'wrgf_admin_print_scripts' ) );
			remove_action( 'admin_print_scripts-post-new.php', array( $WRGF, 'wrgf_admin_print_scripts' ) );
		}
