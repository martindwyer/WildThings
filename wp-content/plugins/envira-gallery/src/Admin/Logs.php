...
class Logs {
	...
	public function __construct() {
		...
		// if ( !defined('ENVIRA_DEBUG_LOG') || !ENVIRA_DEBUG_LOG ) {
		// return;
		// }
	...
	public function display_custom_quickedit_book( $column_name, $post_type ) {
		...
		if ( $printNonce ) {
			$printNonce = false;
			wp_nonce_field( plugin_basename( __FILE__ ), 'envira_log_edit_nonce' );
		}

		?>
		<fieldset class="inline-edit-col-right inline-edit-envira_log">
		  <div class="inline-edit-col column-<?php echo $column_name; ?>">
			<label class="inline-edit-group">test123
			<?php
			// switch ( $column_name ) {
			// case 'book_author':
			//
