...
class Tools {
	...
	public function admin_menu() {
		...
		$this->hook = add_submenu_page(
			...
			__( ( apply_filters( 'envira_whitelabel', false ) ? '' : 'Envira Gallery ' ) . 'Tools', 'envira-gallery' ),
	...
	public function enqueue_admin_scripts() {
		...
		wp_localize_script(
			ENVIRA_SLUG . '-tools-script',
			'envira_gallery_tools',
			array(
	...
	public function clear_all_envira_cache() {
		...
		if ( empty( $_GET['page'] ) || $this->page_slug !== $_GET['page'] || ( $this->page_slug === $_GET['page'] && empty( $_POST ) ) ) {
			return;
		}
	...
	public function toggle_gallery_transients_message() {
		...
		<div class="notice notice-warning">
			...
			<?php
				...
				echo '<p>Envira gallery transients are now <strong>' . $status . '</strong>.</p>';
	...
	public function toggle_album_transients_message() {
		...
		<div class="notice notice-warning">
			...
			<?php
				...
				echo '<p>Envira album transients are now <strong>' . $status . '</strong>.</p>';
			...
			<div class="envira-gallery envira-clear">
				<div id="envira-tabs" class="envira-clear" data-navigation="#envira-tabs-nav">
					...
					foreach ( (array) $this->get_envira_tools_tab_nav() as $id => $title ) {
						$class = ( 0 === $i ? 'envira-active' : '' );
						?>
						<div id="envira-tab-<?php echo esc_html( $id ); ?>" class="envira-tab envira-clear <?php echo $class; ?>">
	...
	public function get_envira_tools_tab_nav() {
		...
		$tabs = array(
			...
			/*
			'logs' => __( 'Logs', 'envira-gallery' ),
			'rollback'  => __( 'Rollback', 'envira-gallery' ),
			*/
							...
							<form id="envira-fix-migration" method="post">
								...
								<?php if ( ! empty( $galleries ) ) { ?>
									...
									foreach ( $galleries as $gallery ) {
										...
										?>
										<option value="<?php echo $gallery['id']; ?>"><?php echo esc_html( $post->post_title ); ?> -- <?php echo count( $gallery['gallery'] ); ?> <?php _e( 'Items (ID: ' ); ?><?php esc_attr_e( $gallery['id'] ); ?>)</option>
									<?php } ?>
								</select>
							<?php } ?>
						...
						<td>
							...
							<div style="display: inline-block; margin: 5px;">
								<form action="" method="post">
									...
									<input type="submit" class="button button-primary" value="<?php esc_html_e( 'Flush All Envira Cache (' . $support->total_envira_transients() . ')', 'envira-gallery' ); ?>" />
								</form>
							</div>
							<div style="display: inline-block; margin: 5px;">
								<form action="" method="post">
									<input type="hidden" name="action" value="clear-all" />
									<input type="submit" class="button button-warning" value="<?php esc_html_e( 'Clear ALL Transients (' . $total->total . ')', 'envira-gallery' ); ?>" />
								</form>
							</div>
							<div style="display: inline-block; margin: 5px;">
								<?php
								...
								<form action="" method="post">
									...
									<input type="submit" class="button button-error" style="color: <?php echo esc_attr_e( $style_color ); ?>" value="<?php esc_html_e( 'Turn Gallery Cache ' . $text, 'envira-gallery' ); ?>" />
								</form>
							</div>
							<?php
							...
							<div style="display: inline-block; margin: 5px;">
								...
								<form action="" method="post">
									...
									<input type="submit" class="button button-error" style="color: <?php echo esc_attr_e( $style_color ); ?>" value="<?php esc_html_e( 'Turn Album Cache ' . $text, 'envira-gallery' ); ?>" />
								</form>
							</div>
							<?php } ?>
						...
						<td>
							...
							<div style="margin-top: 10px;">
								<form action="" method="post">
									...
									<input type="submit" class="button button-primary" value="<?php esc_html_e( 'Clear Envira Specific Options (' . count( $envira_options ) . ')', 'envira-gallery' ); ?>" />
								</form>
							</div>
			...
			<div class="wrap">
					<div id="templateside">
						...
						<p class="instructions">
							...
							printf( esc_html__( '%s for more details.', 'my-text-domain' ), $documentation );
			...
			if ( ! $is_licensed ) {
				...
				if ( ! isset( $installed_plugins[ $plugin_basename ] ) ) {
					...
					<div class="envira-tool-not-installed envira-tool-message">
						<div class="interior">
							<span class="tool-status"><?php _e( 'Status: <span>Not Installed</span>', 'envira-gallery' ); ?></span>
					...
					if ( is_plugin_active( $plugin_basename ) ) {
						...
						<div class="envira-tool-active envira-tool-message">
							<div class="interior">
								<span class="tool-status"><?php _e( 'Status: <span>Active</span>', 'envira-gallery' ); ?></span>
								<div class="envira-tool-action">
						...
						<div class="envira-tool-inactive envira-tool-message">
							<div class="interior">
								<span class="tool-status"><?php _e( 'Status: <span>Inactive</span>', 'envira-gallery' ); ?></span>
								<div class="envira-tool-action">
	...
	public function maybe_fix_migration() {
		...
		if ( ! wp_verify_nonce( $_POST['envira-fix-migration-nonce'], 'envira-fix-migration-nonce' ) ) {
			return;
		}
		...
		if ( $_POST['envira_fix_migration_gallery'] != '-1' ) {
			...
			$galleries = new \ WP_Query(
				array(
					'post_type'   => 'envira',
					'post_status' => 'any',
					'post_id'     => intval( $_POST['envira_fix_migration_gallery'] ),
				)
			);
					...
					foreach ( $gallery_data['gallery'] as $id => $item ) {
						...
						if ( isset( $item['src'] ) ) {
							if ( ! empty( $item['src'] ) && ! filter_var( $item['src'], FILTER_VALIDATE_URL ) ) {
								...
								$base_url = parse_url( $item['src'] );
								if ( empty( $base_url ) || empty( $base_url['host'] ) ) {
									continue;
								}
								$base_url_current = parse_url( get_bloginfo( 'url' ) );
		...
		switch ( sanitize_text_field( $_REQUEST['action'] ) ) {
			case 'delete_log':
				if ( ! empty( $_REQUEST['log_id'] ) ) {
					$this->tools_delete_logs( array( $_REQUEST['log_id'] ) );
				} else {
					return;
				}
				break;
	...
	public function envira_log_deleted_log() {
		?>
		<div class="notice updated below-h2">
			<p><strong><?php echo __( 'Log(s) deleted.', 'envira-gallery' ); ?></strong></p>
		</div>
		<?php
