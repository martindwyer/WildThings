...
class Metaboxes {
	...
	public function notice_warnings() {
		...
		if ( isset( $_GET['envira_slug_exists'] ) ) {
			...
			if ( $duplicate_post_id ) {
				...
				$message = sprintf( __( '<strong>Envira Gallery</strong>: There was <a target="_blank" href="' . $duplicate_post . '">already a post on your site</a> with the same slug. Envira generated a unique slug for this gallery.</a>', 'envira-gallery' ) );
			} else {
				// Add a notice for the user that this changed, but saving anyway
		...
		<li id="<?php echo $id; ?>" class="envira-gallery-image envira-gallery-status-<?php echo $item['status']; ?>" data-envira-gallery-image="<?php echo $id; ?>" data-envira-gallery-image-model='<?php echo htmlspecialchars( $json, ENT_QUOTES, 'UTF-8' ); ?>'>
			...
			<div class="additional additional-list">
				<?php
					...
					echo apply_filters( 'envira_gallery_metabox_output_gallery_item_meta', '', $item, $id, $post_id, 'list' );
				?>
			</div>
			<div class="meta meta-grid">
				...
				<div class="additional additional-grid">
					<?php
					// Addons can add content to this meta section, which is displayed when in the List View.
					echo apply_filters( 'envira_gallery_metabox_output_gallery_item_meta', '', $item, $id, $post_id, 'grid' );
					?>
						...
						<tr id="envira-config-additional-copy-box-automatic">
							...
							<td>
								...
								foreach ( envira_get_additional_copy_options() as $option_value => $option_name ) {
									...
									if ( empty( $option_value ) || $option_name == 'None' ) { // no need to show 'None' for checkboxes
										continue;
									}
				...
				<table class="form-table">
					...
					<tbody>
						...
						<tr id="envira-config-gutter-box">
							...
							<td>
								<input id="envira-config-gutter" type="number" name="_envira_gallery[gutter]" value="<?php echo envira_get_config( 'gutter', $gallery_data, envira_get_config_default( 'gutter' ) ); ?>" /> <span class="envira-unit"><?php esc_html_e( 'px', 'envira-gallery' ); ?></span>
						...
						<tr id="envira-config-margin-box">
							...
							<td>
								<input id="envira-config-margin" type="number" name="_envira_gallery[margin]" value="<?php echo envira_get_config( 'margin', $gallery_data, envira_get_config_default( 'margin' ) ); ?>" /> <span class="envira-unit"><?php esc_html_e( 'px', 'envira-gallery' ); ?></span>
			...
			<table class="form-table no-margin">
				<tbody>
					<tr id="envira-config-lightbox-enabled-box">
						...
						<td>
							<input id="envira-config-lightbox-enabled" type="checkbox" name="_envira_gallery[lightbox_enabled]" value="<?php echo envira_get_config( 'lightbox_enabled', $gallery_data ); ?>" <?php checked( envira_get_config( 'lightbox_enabled', $gallery_data ), 1 ); ?> />
					...
					<tbody>
						...
						<tr id="envira-config-lightbox-additional-title-caption">
							...
							<td>
								...
								<p class="description"><?php _e( 'Caption is the default text, but you can choose title instead or nothing at all (for maximum size images with supersize).', 'envira-gallery' ); ?></p>
							</td>
						</tr>
					...
					<table class="form-table">
						<tbody>
							...
							<tr id="envira-config-thumbnails-custom-size">
								<th scope="row">
									<label for="envira-config-thumbnails-custom-size"><?php _e( 'Use Custom Width/Height?', 'envira-gallery' ); ?></label>
								</th>
								<td>
									<input id="envira-config-thumbnails-custom-size" type="checkbox" name="_envira_gallery[thumbnails_custom_size]" value="<?php echo envira_get_config( 'thumbnails_custom_size', $gallery_data ); ?>" <?php echo $checked; ?> />
									<span class="description"><?php _e( 'This enables you to enter a custom width and height, overriding Envira\'s automatic settings.', 'envira-gallery' ); ?></span>
								</td>
							</tr>
							...
							<tr id="envira-config-thumbnail-hide">
								<th scope="row">
									<label for="envira-config-thumbnail-hide"><?php _e( 'Hide Thumbnails On Opening Lightbox?', 'envira-gallery' ); ?></label>
								</th>
								<td>
									<input id="envira-config-thumbnail-hide" type="checkbox" name="_envira_gallery[thumbnails_hide]" value="<?php echo envira_get_config( 'thumbnails_hide', $gallery_data ); ?>" <?php checked( envira_get_config( 'thumbnails_hide', $gallery_data ), 1 ); ?> />
									<span class="description"><?php _e( 'Enables thumbnails to stay initially hidden when the lightbox opens. User must click the thumbnail button to make them visible.', 'envira-gallery' ); ?></span>
								</td>
							</tr>
					...
					<tr id="envira-config-title-caption-automatic-mobile">
							...
							<td>
								...
								foreach ( envira_get_additional_copy_options() as $option_value => $option_name ) {
									...
									?>
									<label for="envira-config-<?php echo $option_value; ?>-automatic-mobile" class="label-for-checkbox">
										<input id="envira-config-<?php echo $option_value; ?>-automatic-mobile" type="checkbox" name="_envira_gallery[additional_copy_automatic_<?php echo $option_value; ?>_mobile]" value="1" <?php checked( envira_get_config( 'additional_copy_automatic_' . $option_value . '_mobile', $gallery_data, envira_get_config_default( 'additional_copy_automatic_' . $option_value . '_mobile' ) ), 1 ); ?> />
										<?php echo $option_name; ?>
									</label>
									<?php
								}
						...
						<tr id="envira-config-title-caption-column-mobile">
							...
							<td>
								...
								foreach ( envira_get_additional_copy_options() as $option_value => $option_name ) {
									...
									?>
									<label for="envira-config-<?php echo $option_value; ?>-mobile" class="label-for-checkbox">
										<input id="envira-config-<?php echo $option_value; ?>-mobile" type="checkbox" name="_envira_gallery[additional_copy_<?php echo $option_value; ?>_mobile]" value="1" <?php checked( envira_get_config( 'additional_copy_' . $option_value . '_mobile', $gallery_data, envira_get_config_default( 'additional_copy_' . $option_value . '_mobile' ) ), 1 ); ?> />
										<?php echo $option_name; ?>
									</label>
				...
				<tbody>
					<tr id="envira-config-standalone-box">
							...
							<td>
								...
								<p class="description"><?php _e( 'Your current theme does not have any custom templates. If you want to use a template besides the default, you need to add a custom template to your theme.', 'envira-gallery' ); ?></p>
			...
			<table class="form-table">
				<tbody>
					...
					<tr id="envira-config-export-box">
						<th scope="row">
							<label for="envira-config-import-gallery"><?php _e( 'Export Gallery', 'envira-gallery' ); ?></label>
						</th>
						<td>
	...
	public function save_meta_boxes( $post_id, $post ) {
		...
		$settings['config']['random']               = ( isset( $_POST['_envira_gallery']['random'] ) ) ? preg_replace( '#[^a-z0-9-_]#', '', $_POST['_envira_gallery']['random'] ) : false;
		$settings['config']['sort_order']           = ( isset( $_POST['_envira_gallery']['random'] ) ) ? preg_replace( '#[^a-z0-9-_]#', '', $_POST['_envira_gallery']['random'] ) : false;
		$settings['config']['sorting_direction']    = isset( $_POST['_envira_gallery']['sorting_direction'] ) ? sanitize_text_field( $_POST['_envira_gallery']['sorting_direction'] ) : false;
		...
		$slug_to_save = $settings['config']['slug'] = ( ! empty( $_POST['_envira_gallery']['slug'] ) ) ? sanitize_title( $_POST['_envira_gallery']['slug'] ) : sanitize_title( $post->post_name );
