...
class Shortcode {
	...
	public function shortcode( $atts ) {
		...
		if ( $markup && ( ! defined( 'ENVIRA_DEBUG' ) || ! ENVIRA_DEBUG ) ) {
			...
			if ( $gallery_images_raw === false ) {
				$gallery_images_raw = envira_get_gallery_images( $gallery_id, true, $data, false, false, $gallery_type );
			}

			// print_r ($gallery_images_raw); exit;
			$fix_json = array();
	...
	public function generate_gallery_item_markup( $gallery, $data, $item, $id, $i, $images = false ) {
		...
		if (
			( $gallery_theme == 'captioned' || $gallery_theme == 'polaroid' ) &&
			 (
				 ...
				 ||
				 ( $this->is_mobile && envira_get_config( 'additional_copy_caption_mobile', $data ) && ( envira_get_config( 'columns', $data ) != 0 ) )
			)
		) {
		...
		if ( false !== $create_link ) {
			...
			$envia_gallery_link = envira_get_config( 'lightbox_enabled', $data ) && empty( $data['config']['instagram_link'] ) ? 'envira-gallery-link' : '';
